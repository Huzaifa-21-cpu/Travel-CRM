  ...
  return (
    <div className="mx-auto max-w-2xl space-y-5 p-6">
      <div>
        ...
        <h1 className="mt-0.5 text-lg font-semibold text-stone-900">{booking.bookingRef}</h1>
        <p className="text-xs text-stone-500">
          {booking.customer.name} · {booking.customer.phone}
        </p>
      </div>

      <div className="grid grid-cols-3 gap-3 rounded-xl border border-stone-200 bg-white p-4 shadow-sm text-sm">
        <div>
          <p className="text-xs text-stone-400">Total</p>
          <p className="text-stone-900">${booking.totalAmount.toLocaleString()}</p>
        </div>
        <div>
        ...
        <div>
          ...
          <p className={balance > 0 ? "text-amber-600" : "text-stone-900"}>
      ...
      <div className="rounded-xl border border-stone-200 bg-white p-4 shadow-sm">
        ...
        <ul className="mt-3 divide-y divide-stone-100">
          {booking.payments.map((p) => (
            <li key={p.id} className="flex items-center justify-between py-2 text-sm">
              ...
              <span className="flex items-center gap-2">
                ...
                <span className="font-medium text-stone-900">${p.amount.toLocaleString()}</span>
          ...
          {booking.payments.length === 0 && (
            <li className="py-2 text-xs text-stone-400">No payments recorded yet.</li>
          )}
        </ul>
      </div>
    </div>
